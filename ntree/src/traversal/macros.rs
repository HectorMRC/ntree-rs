...
pub(super) mod r#async {
    ...
    macro_rules! for_each_immersion {
        ($node:ty, $getter:tt) => {
            #[async_recursion]
            pub async fn for_each_immersion<O, F, T>(root: $node, f: &F)
            where
                T: Sync + Send,
                F: Fn($node) + Sync + Send,
                O: Order,
            {
                let mut todo: Vec<$node> = Vec::with_capacity(root.children.len());
                for it in 0..=root.children.len() {
                    if O::evaluate_self(root, it) {
                        // evaluate the pre-selected children, if any
                        join_all(
                            todo.iter()
                                .map(|item| for_each_immersion::<O, F, T>(item, f)),
                        )
                        .await;
                ...
                // evaluate the post-selected children, if any
                join_all(
                    todo.iter()
                        .map(|item| for_each_immersion::<O, F, T>(item, f)),
                )
